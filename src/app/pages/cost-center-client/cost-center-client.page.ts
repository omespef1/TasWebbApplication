...
@Component({
  selector: 'app-cost-center-client',
  templateUrl: './cost-center-client.page.html',
  styleUrls: ['./cost-center-client.page.scss'],
})
export class CostCenterClientPage implements OnInit {
...
if(event.detail.value.length>0){
  ...
  getCostCeter() {
    console.log(event);
    this.loading = true;
    this.gesCentroCostosService.GetCostCenterByClientPasseenger(this.sesionService.GetUser().IdEmpresa, this.sesionService.GetUser().Identificacion).subscribe(resp => {
      this.loading = false;
      //console.log(resp);
