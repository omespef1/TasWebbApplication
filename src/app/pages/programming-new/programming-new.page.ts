...
loading=false;
  constructor(private costCenterService: GescentrocostosService, private session: SessionService, private modal: ModalController,
    ...
    private nav:NavController,private router:Router,private changes:ChangeDetectorRef) { 
...
console.log(this.request);
 ...
 getlocation(){
  ...
  addDetailsInit() {
    ...
    if(this.request.SolicitudId==0 && this.request.GESSolicitudServiciosDetalle.length== 0){
      this.request.GESSolicitudServiciosDetalle.push({ SolicitudId: 0, EmpresaId: this.session.GetThirdPartie().IdEmpresa, Kilometraje: 0, Estado: 'I', Latitude: this.latitude, Longitude:  this.longitude, observations: '', Id: 0, UsuarioCrea: '', UsuarioModifica: '', FechaCrea: new Date(), FechaModifica: new Date() })
      this.request.GESSolicitudServiciosDetalle.push({ SolicitudId: 0, EmpresaId: this.session.GetThirdPartie().IdEmpresa, Kilometraje: 0, Estado: 'O', Latitude: this.latitude, Longitude:  this.longitude, observations: '', Id: 2, UsuarioCrea: '', UsuarioModifica: '', FechaCrea: new Date(), FechaModifica: new Date() })
      this.request.GESSolicitudServiciosDetalle.push({ SolicitudId: 0, EmpresaId: this.session.GetThirdPartie().IdEmpresa, Kilometraje: 0, Estado: 'R', Latitude: this.latitude, Longitude:  this.longitude, observations: '', Id: 3, UsuarioCrea: '', UsuarioModifica: '', FechaCrea: new Date(), FechaModifica: new Date() })  
    }
  ...
  addDetailsEnd() {
    ...
    if(this.request.SolicitudId>0 && this.request.GESSolicitudServiciosDetalle.length== 3){
  ...
  checkApprovedLicensePlate() {
    return this.genTercerosService.IsAuthorizedForService(this.session.GetThirdPartie().IdEmpresa, this.session.GetThirdPartie().IdTercero).subscribe(resp => {
...
this.loading = true;
  ...
  return  this.requestService.CheckPendingServices(this.session.GetThirdPartie().IdEmpresa,this.session.GetThirdPartie().IdTercero).subscribe(resp=>{
      if(resp!=undefined && resp.Retorno==0){
